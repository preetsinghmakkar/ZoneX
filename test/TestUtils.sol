...
abstract contract TestUtils is Test, Assertions {
    ...
    function encodeExtra(address token0_, address token1_, address payer) internal pure returns (bytes memory) {
        return abi.encode(IZoneXPool.CallbackData({token0: token0_, token1: token1_, payer: payer}));
    }
    ...
    function mintParams(
        ...
        params = IZoneXManager.MintParams({
            tokenA: tokenA,
            tokenB: tokenB,
            fee: 3000,
    ...
    function deployPool(ZoneXPoolFactory factory, address token0, address token1, uint24 fee, uint256 currentPrice)
        internal
        returns (ZoneXPool pool)
    {
        pool = ZoneXPool(factory.createPool(token0, token1, fee));
